# Imports 
# if you get an error saying module not found you need you use pip install {name of module} to install
import pymongo
...
# functions we wrote and stored in helper_functions.py being imported into this file for us to use
...
# using the os lib to load our mongoDB password
...
# Code for hanlding user creation and database 

# An empty dictionary that we will use to store use info

User_df = {'Username':[],'Userlocation':[],'Usernumber':[],
                          'Useremail':[],'Usertype':[],'Usercity':[]}
class newUser :          
    """A class for new users that takes in inputs and creates {newUserid, location, latitude, longitude, point},
    from them and stores all data in the Details object """
...
def add_user_data(newUsertest):
    """
    This function adds our user data into our mongodb
    using the values inside our {newUser.Details} class instance as pd df.
    """    
        ...
        if LoginNav == 'Sign in to ride':
            ...
            if st.checkbox('Enter Rider Accounr'):
                ...
                if len(user_check) != 0 and len(user_checkTwo) != 0:
                    ...
                    selected_drivers_slice = selected_drivers[['driver_name', 'driver_email', 'driver_number', 'taxy_id','geohash']] # 'geohash'
